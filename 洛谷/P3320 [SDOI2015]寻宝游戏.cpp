 ...
 *    created: 2023.02.01 周三 21:11:56 (Asia/Shanghai)
 **/
#include <algorithm>
#include <cstdio>
#include <set>
...
signed main()
{
    ...
    for (int i = 1; i <= m; ++i)
    {
        ...
        if (!exist[x])
        {
            ...
            exist[x] = true;
            set<int>::iterator it;
            y = rk[(it = se.lower_bound(dfn[x])) == se.begin() ? *--se.end() : *--it];
            z = rk[(it = se.upper_bound(dfn[x])) == se.end() ? *se.begin() : *it];
            ans += getdis(x, y) + getdis(x, z) - getdis(y, z);
        }
        else
        {
            se.erase(dfn[x]);
