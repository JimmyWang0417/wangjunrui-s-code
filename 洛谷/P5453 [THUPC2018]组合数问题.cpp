...
inline int get_num_of_skill(string s)
{
	if (s == "toolihai")
		return 1;
	if (s == "faceking")
...
inline void init_one(int i)
{
	nown[i] = a[i];
	nowm[i] = b[i];
	skillable[i] = 0;
	bombed[i] = 0;
	while (enforce[i].size()) enforce[i].pop();
	dead[i] = 0;
}
...
inline void init()
{
	...
	for (re int i = 1; i <= n; i++)
		for (re int j = 1; j <= m; j++) read(map[i][j]);
	...
	for (re int i = 0; i <= n + 1; i++) map[i][0] = map[i][m + 1] = 1;
	for (re int i = 0; i <= m + 1; i++) map[0][i] = map[n + 1][i] = 1;

	lastp = 0;
}

inline void respawn(int i)
{
...
inline bool check_in(int j, int i)
{
	int ndx = x[j] - x[i];
	int ndy = y[j] - y[i];
	if (d[i] == 0)
			...
			else if (s[i] == 6)
			{
				...
				for (int j = 1; j <= k; j++)
					if (dead[j])
					{
						...
						if (c[i] == c[j])
						{
							if (!maxp || distance >= dis(maxp, i))
								maxp = j;
						}
						else
						{
							if (!minp || distance <= dis(minp, i))
								minp = j;
						}
					}
				if (maxp)
...
inline void die(int i)
{
	dead[i] = r[i];
}

inline long double combinational(int i)
{
...
inline void fight()
{
	...
	for (re int i = 1; i <= k; i++) id[i] = i;
	sort(id + 1, id + k + 1, cmp);
	for (re int i = 1; i <= k;)
	{
		int j = i;
		while (j <= k && x[id[j]] == x[id[i]] && y[id[j]] == y[id[i]]) j++;
...
{
	for (re int now_round = 1; now_round <= R; now_round++)
	{
	...
	for (re int i = 1; i <= k; i++) printf("%d %d\n", x[i], y[i]);
}
...
int main()
{
	...
	dy[2] = -1;
	dy[3] = 1;
	for (re int a = 1; a <= 1000000; a++) fac[a] = fac[a - 1] + logl(a);
	int T;
	read(T);
