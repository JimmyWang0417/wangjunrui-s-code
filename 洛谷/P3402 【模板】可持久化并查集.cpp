...
private:
    ...
    {
        rt = ++cnt;
        tree[rt].lc = tree[last].lc;
        tree[rt].rc = tree[last].rc;
        if (l == r)
        {
            tree[rt].fa = fa;
        ...
        if (pos <= mid)
            ...
            merge(tree[last].rc, tree[rt].rc, mid + 1, r, pos, fa);
    }
    ...
    inline int query(int rt, int l, int r, int pos)
    {
        ...
        int mid = (l + r) >> 1;
        if (pos <= mid)
            return query(tree[rt].lc, l, mid, pos);
        else
            return query(tree[rt].rc, mid + 1, r, pos);
    }
