 ...
 *    created: 2022.09.27 周二 17:58:34 (Asia/Shanghai)
 **/
#include <algorithm>
#include <cstdio>
#include <vector>
...
signed main()
{
    ...
    for (int i = 1; i <= k; ++i)
    {
        ...
        for (auto j : g[i])
        {
            if (j.second < r)
            {
                h[j.second].emplace_back(i, j.first);
                r = j.second;
            }
        }
    }
    ...
    for (int r = 1; r <= n; ++r)
    {
        dp[r] = (int)((ll)dp[r - 1] * k % mod);
        for (auto j : h[r])
        {
            auto [v, l] = j;
            int res = dp[l - 1];
            del(res, query(root[v], 1, n, l, r - 1));
            del(dp[r], res);
            update(root[v], 1, n, r, res);
        }
    }
    printf("%d\n", dp[n]);
#ifdef FAST_OUT
