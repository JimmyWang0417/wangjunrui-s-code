 ...
 *    created: 2022.09.25 周日 15:24:10 (Asia/Shanghai)
 **/
#include <cstdio>
#define ll long long
#define lll __int128
...
inline void init(int limit)
{
    ...
    for (int i = 1; i <= limit; ++i)
        fac[i] = fac[i - 1] * i % mod;
    ifac[limit] = quickpow(fac[limit], mod - 2);
...
signed main()
{
    ...
    while (T--)
    {
        ...
        ll ans = 0;
        ll qwq = 2;
        for (int i = 0; i < n - 1; ++i, (qwq *= 2) %= mod)
            (ans += C(n - 1, i) * C(n - 1, i + 1) % mod * qwq) %= mod;
        write((fac[n] - ans * ifac[n - 1] % mod * fac[n - 2] % mod + mod) % mod, '\n');
    }
#ifdef FAST_OUT
    flush();
#endif
