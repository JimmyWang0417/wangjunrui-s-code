 ...
 *    created: 2022.09.24 周六 08:26:53 (Asia/Shanghai)
 **/
#include <cstdio>
#include <vector>
#define ll long long
...
int n, m, limit;
int power[N];
inline void dfs(int l, int r, int len)
{
    for (int i = l; i < l + len; ++i)
        for (int j = 0; j < len; ++j)
        {
            for (int k = 0; k < n; ++k)
                write(l + k * len + (i - l + j * k) % len, ' ');
            write('\n');
        }
    if (len > 1)
        for (int i = 0; i < n; ++i)
            dfs(l + i * len, l + (i + 1) * len, len / n);
}
signed main()
{
