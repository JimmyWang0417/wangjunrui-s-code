...
constexpr int ALL = (1 << 20) + 5;
int n, m;
int a[N][M], b[N][N][M];
int dp[ALL][N];
int Log[ALL];
inline void solve(int *_dp)
{
    for (int i = m - 1; i >= 0; --i)
        ckmin(_dp[i], _dp[(i + 1) % m] + 1);
    for (int i = m - 1; i >= 0; --i)
        ckmin(_dp[i], _dp[(i + 1) % m] + 1);
}
signed main()
{
    ...
    int all = (1 << n) - 1;
    for (int S = 1; S <= all; ++S)
        for (int _S = S, i; i = Log[lowbit(_S)], _S; _S ^= 1 << i)
            if (dp[S][i] < 0x3f3f3f3f)
                for (int _T = all ^ _S, j; j = Log[lowbit(_T)], _T; _T ^= 1 << j)
                {
                    ckmin(dp[S | (1 << j)][j], dp[S][i] + b[i][j][dp[S][i] % m] +
                                                   a[j][(dp[S][i] + b[i][j][dp[S][i] % m]) % m]);
                }
    int q;
    read(q);
    while (q--)
