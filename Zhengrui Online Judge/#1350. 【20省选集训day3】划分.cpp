...
signed main()
{
    ...
    for (int i = 1; i <= n; ++i)
        b[i] = a[i];
    m = (int)(unique(b + 1, b + 1 + n) - b - 1);
    for (int i = 1; i <= m; ++i)
        pre[i] = pre[i - 1] + b[i];
    for (int i = 1; i < m; ++i)
    {
        c[i] = __gcd(b[i], b[m]);
        if (top && b[st[top]] - c[st[top]] <= b[i] - c[i])
            continue;
        st[++top] = i;
    }
    ll gcd = b[m];
    for (int i = m - 1; i >= 1; --i)
    {
        while (top && st[top] > i)
            --top;
        pre[i] -= b[st[top]] - c[st[top]];
        ll tmp = __gcd(b[i], gcd);
        if (tmp != gcd)
        {
            top = 0;
            gcd = tmp;
            ...
            {
                c[j] = __gcd(c[j], gcd);
                if (top && b[st[top]] - c[st[top]] <= b[j] - c[j])
                    continue;
                st[++top] = j;
            }
        }
