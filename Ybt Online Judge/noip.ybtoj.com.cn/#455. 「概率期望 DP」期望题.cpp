/**
 *    unicode: UTF-8
 *    name:    #455. 「概率期望 DP」期望题
 *    author:  whitepaperdog (蒟蒻wjr)
 *    located: Changle District, Fuzhou City, Fujian Province, China
 *    created: 2023.02.04 周六 19:27:56 (Asia/Shanghai)
 **/
#include <cstdio>
...
int dep[N];
node inv[N];
node dp[N], f[N], g[N], h[N];
inline node dfs(int u, int _fa)
{
    if (dep[u] == 1)
    {
        dp[u] = (str[u] == '1');
    ...
    node times = ((node)1 - res * inv[dep[u]]).inv();
    dp[u] = (dp[u] * inv[dep[u]] + (str[u] == '1')) * times;
    return inv[dep[u]] * times;
}
inline void dfs1(int u, int _fa)
{
    ...
    h[u] = inv[dep[u]] / ((node)1 - res * inv[dep[u]]);
}
inline void dfs2(int u, int _fa)
{
    ...
    for (int i = head[u]; i; i = edge[i].next)
    {
        ...
        if (v == _fa)
            continue;
        g[v] = inv[dep[u]] / ((node)1 - (res - h[v]) * inv[dep[u]]);
        f[v] = f[u] * g[v];
        dfs2(v, u);
    }
}
