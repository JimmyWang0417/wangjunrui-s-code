 ...
 *    created: 2023.02.01 周三 15:53:26 (Asia/Shanghai)
 **/
#include <cstdio>
...
node fac[N], ifac[N], power[N + (int)1e6];
signed main()
{
#ifdef PAPERDOG
    ...
    power[0] = (node)(s + 1) ^ ((ll)(w + 1) * (h + 1));
    node times = ((node)s + 1).inv();
    limit = 2 * (h + w) + 1;
    for (int i = 1; i <= limit; ++i)
        power[i] = power[i - 1] * times;
    node ans = 0;
    for (int a = 0; a < h; ++a)
    {
        int b = w,
            c = h - a,
            d = 2 * (a + b) + c + 1;
        ans += power[d] * C(a + b - 1, a) * C(s + a + 2 * b + c - 1, 2 * (a + b) + c - 1);
    }
    for (int b = 0; b < w; ++b)
    {
        int a = h,
        ...
        ans += power[d] * C(a + b - 1, b) * C(s + a + 2 * b + c - 1, 2 * (a + b) + c - 1);
    }
    write(ans.data(), '\n');
#ifdef FAST_OUT
    IO::OUTPUT::flush();
