 ...
 *    created:  2023.02.11 周六 08:29:02 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <cstdio>
...
inline node calc(int x)
{
    if (x == 1)
        return 1;
    matrix f = (g ^ (x - 1));
    return f.g[0][0] + f.g[1][0] * 2;
}
inline node dfs(int pos = 1, int now = 1, int phi = 1)
{
    if (pos > tot)
        return calc(now) * phi;
    int lasphi = phi;
    ...
    for (int i = 1; i < q[pos]; ++i)
        phi *= p[pos];
    node res = 0;
    for (int i = 0; i < q[pos]; ++i, phi /= p[pos], now *= p[pos])
        res += dfs(pos + 1, now, phi);
    res += dfs(pos + 1, now, lasphi);
    return res;
}
signed main()
{
