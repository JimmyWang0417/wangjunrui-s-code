 ...
 *    created:  2023.02.11 周六 15:00:33 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <algorithm>
...
signed main()
{
    ...
    for (int i = 1; i <= n; ++i)
        p[i] = p[i - 1] * 0.5;
    double div = 1 - ((double)n * (n - 1) / 2 + n + 1) * p[n];
    double S = 0, cnt = 0;
    for (int l = 1; l <= n; ++l)
    {
        for (int i = 2, r = l + 1; i < n && i <= 60; ++r, ++i)
        {
            if (r > n)
                r -= n;
            double times = (p[i] - p[n]) / div;
            S += a[l] * a[r] * times;
            cnt += (a[r] - a[l]).calc() * times;
        }
    }
    printf("%.10lf\n", (S - cnt) / 2 + 1);
#ifdef FAST_OUT
