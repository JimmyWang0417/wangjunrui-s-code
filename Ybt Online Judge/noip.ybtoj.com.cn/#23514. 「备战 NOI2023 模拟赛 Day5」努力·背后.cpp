 ...
 *    created:  2023.03.25 周六 21:09:49 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <algorithm>
...
inline void init(int u)
{
    ...
    for (int i = head[u]; i; i = edge[i].next)
    {
        int v = edge[i].to;
        init(v);
        lasroot[u] = merge(lasroot[u], root[v], 1, n);
        sze[u] += sze[v];
    }
    update(lasroot[u], root[u], 1, n, sze[u]);
}
int answer[N];
inline void dfs(int u)
{
    int minsze = 0x7fffffff;
    int maxsze = 0, maxpos = 0, nxtsze = 0;
    for (int i = head[u]; i; i = edge[i].next)
    {
        int v = edge[i].to;
        ckmin(minsze, sze[v]);
    ...
    if (maxsze < n - sze[u])
    {
        ...
        ckmax(nxtsze, n - sze[u]);
    auto check = [&](int mid) -> bool
    {
        int l = maxsze - mid, r = min(n, mid - minsze);
        if (l <= 0)
            return true;
        if (l > r)
            return false;
        if (maxpos == fa[u])
            return query(root[1], 1, n, l, r) - query(root[u], 1, n, l, r) -
                       query(l, r) + query(l + sze[u], r + sze[u]) >
                   0;
        else
            return query(lasroot[maxpos], 1, n, l, r) > 0;
    };
    int l = nxtsze, r = maxsze;
    while (l <= r)
    {
        int mid = (l + r) >> 1;
        if (check(mid))
