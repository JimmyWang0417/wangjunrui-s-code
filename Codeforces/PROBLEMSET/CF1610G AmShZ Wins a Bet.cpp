/**
 *    name:     CF1610G AmShZ Wins a Bet
 *    author:   whitepaperdog (蒟蒻wjr)
 *    located:  Changle District, Fuzhou City, Fujian Province, China
 *    created:  2023.02.20 周一 17:13:43 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <cstdio>
...
inline int insert(int u, int c)
{
    ...
    if (!v)
    {
        ...
        {
            f[v][i + 1] = f[f[v][i]][i];
            g[v][i + 1] = g[f[v][i]][i] * power[i] + g[v][i];
        }
    }
    return v;
}
...
signed main()
{
    ...
    for (int i = n; i >= 1; --i)
    {
        if (str[i] == '(')
        {
            int u = insert(dp[i + 1], 0);
            if (top)
            {
                if (compare(u, dp[st[top] + 1]))
                    dp[i] = u;
                else
                    dp[i] = dp[st[top] + 1];
                --top;
        ...
        {
            st[++top] = i;
            dp[i] = insert(dp[i + 1], 1);
        }
    }
    for (int u = dp[1]; u != 1; u = f[u][0])
        write(g[u][0] ? ')' : '(');
    write('\n');
#ifdef FAST_OUT
    IO::OUTPUT::flush();
