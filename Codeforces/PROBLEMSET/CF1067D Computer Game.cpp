/**
 *    name:     CF1067D Computer Game
 *    author:   whitepaperdog (蒟蒻wjr)
 *    located:  Xuanwu District, Nanjing City, Jiangsu Province, China
 *    created:  2023.03.17 周五 20:39:55 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: p++23
 **/
#include <algorithm>
...
#undef putchar
#define putchar(p) (_now - _buf == FAST_OUT_BUFFER_SIZE ? flush(), *_now++ = p : *_now++ = p)
#else
#if !defined(WIN32) && !defined(_WIN32)
#define putchar putchar_unlocked
#endif
...
int n, rk[N], hull[N], tl;
ll t;
double p[N], g[N], a[N], b[N], M;
    ...
    for (int j = 1; j <= n; ++j)
    {
        ...
        else
        {
            while ((g[i] - g[hull[tl]]) * (p[hull[tl]] - p[hull[tl - 1]]) >
                   (g[hull[tl]] - g[hull[tl - 1]]) * (p[i] - p[hull[tl]]))
                tl--;
            hull[++tl] = i;
        }
    }
