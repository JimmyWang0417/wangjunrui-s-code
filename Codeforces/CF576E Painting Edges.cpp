 ...
 *    created: 2023.02.08 周三 19:58:01 (Asia/Shanghai)
 **/
#include <algorithm>
#include <cstdio>
#include <vector>
...
int a[N], b[N], c[N];
pair<int, int> e[N];
int fa[M][N * 2], dep[M][N * 2];
inline int find(int col, int x)
{
...
{
    int x, col, dep;
    node(int _x = 0, int _col = 0, int _dep = 0) : x(_x), col(_col), dep(_dep) {}
} st[N];
int top;
...
inline void unionn(int x, int y, int col)
{
    x = find(col, x), y = find(col, y);
    if (x == y)
        return;
    if (dep[col][x] < dep[col][y])
        swap(x, y);
    st[++top] = node(y, col, dep[col][x]);
    fa[col][y] = x;
    if (dep[col][x] == dep[col][y])
        ++dep[col][x];
}
...
inline void query(int rt, int l, int r)
{
    ...
    while (top > lastop)
    {
        auto [x, col, _dep] = st[top--];
        dep[col][fa[col][x]] = _dep;
        fa[col][x] = 0;
    }
}
signed main()
{
    ...
    for (int i = 1; i <= q; ++i)
    {
        read(b[i], c[i]);
        if (a[b[i]])
            update(1, 1, q, a[b[i]] + 1, i - 1, a[b[i]]);
        a[b[i]] = i;
    }
    for (int i = 1; i <= m; ++i)
        if (a[i])
        {
            update(1, 1, q, a[i] + 1, q, a[i]);
            a[i] = 0;
        }
    query(1, 1, q);
#ifdef FAST_OUT
