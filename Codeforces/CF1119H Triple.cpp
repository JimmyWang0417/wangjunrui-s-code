 ...
 *    created: 2023.01.31 周二 21:32:22 (Asia/Shanghai)
 **/
#include <cstdio>
...
signed main()
{
    ...
    FWT(f), FWT(g), FWT(h);
    node g1 = (node)x + y + z, g2 = (node)x + y - z,
         g3 = (node)x - y + z, g4 = (node)x - y - z;
    for (int i = 0; i < (1 << m); ++i)
    {
        int c1 = (n + f[i] + g[i] + h[i]) / 4,
            c2 = (n + f[i]) / 2 - c1,
            c3 = (n + g[i]) / 2 - c1,
            c4 = (n + h[i]) / 2 - c1;
        dp[i] = (g1 ^ c1) * (g2 ^ c2) * (g3 ^ c3) * (g4 ^ c4);
    }
    IFWT(dp);
    for (int i = 0; i < (1 << m); ++i)
        write(dp[i ^ all].data(), ' ');
