 ...
 *    created:  2023.03.08 周三 11:09:44 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <cstdio>
...
inline void pushup(int rt)
{
    ...
    tree[rt].reach = (tree[lc].reach & tree[rc].reach);
    for (int i = 0; i < 24; ++i)
        tree[rt][i] = tree[lc][i] + tree[rc][i];
}
...
inline void build(int rt, int l, int r)
{
    if (l == r)
    {
        ...
        b[l] = mp[res.data()], c[l] = cnt - mp[res.data()];
        mp.clear();
        if (b[l])
            tree[rt].sum = a[l];
        else
        {
            res = a[l];
            for (int i = 0; i < 24; ++i, res = res * res)
                tree[rt][i] = res;
            tree[rt].reach = true;
        }
        return;
