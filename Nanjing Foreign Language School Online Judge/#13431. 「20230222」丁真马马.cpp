 ...
 *    created:  2023.02.22 周三 09:19:49 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <cstdio>
...
int cnt[N];
int answer[N];
inline bool query(int x, int y, int expect)
{
    int res = (x /= y);
    if (res > expect)
        return false;
...
signed main()
{
...
#else
    freopen("zhenzhu.in", "r", stdin);
    freopen("zhenzhu.out", "w", stdout);
#endif
    read(n, m, q);
    int maxx = 0;
    for (int i = 1; i <= n; ++i)
    ...
    for (int i = 1; i < n; ++i)
    {
        for (int j = 1; i * j + m <= maxx; ++j)
        {
            int x = i * j + m;
            if (!cnt[x])
                continue;
            answer[i + 1] += cnt[x] * query(x, i + 1, j);
        }
    }
    for (int i = 1; i <= q; ++i)
    {
