/**
 *    name:     #13436. 「20230223」大枝江
 *    author:   whitepaperdog (蒟蒻wjr)
 *    located:  Changle District, Fuzhou City, Fujian Province, China
 *    created:  2023.02.23 周四 16:40:17 (Asia/Shanghai)
 *    unicode:  UTF-8
 *    standard: c++23
 **/
#include <algorithm>
...
inline void ckmin(tot &_x, tot _y)
{
    if (_x > _y)
        _x = _y;
}
template <typename tot>
inline void ckmax(tot &_x, tot _y)
{
    if (_x < _y)
        _x = _y;
}
...
signed main()
{
    ...
    do
    {
        ...
        if (tot > ans)
        {
            ...
            tot = 0;
            for (int i = 1; i <= n; i++)
                if ((int)g[p[i]].size() >= limit)
                    p[++tot] = p[i];
            n = tot;
        }
        if (ans >= n)
            break;
    } while (ans < n && (double)(clock() - beg) / CLOCKS_PER_SEC < 3.9);
    write(ans, '\n');
#ifdef FAST_OUT
    IO::OUTPUT::flush();
#endif
